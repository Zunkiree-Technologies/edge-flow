...
exports.getProductionViewDataWithFilter = exports.getProductionViewData = void 0;
 ...
 */
const getProductionViewData = async () => {
    try {
        // 1. Fetch all departments
        const departments = await db_1.default.departments.findMany({
            orderBy: {
                id: 'asc'
            }
        });
        // 2. Fetch all sub-batches with their details
        const allSubBatches = await db_1.default.sub_batches.findMany({
            include: {
                batch: true,
                size_details: true,
                attachments: true,
        ...
        const departmentSubBatches = await db_1.default.department_sub_batches.findMany({
            where: {
                is_current: true,
            },
            include: {
        ...
        const completedSubBatches = await db_1.default.sub_batches.findMany({
            where: {
                status: 'COMPLETED'
            },
            include: {
...
exports.getProductionViewData = getProductionViewData;
/**
 * Get production view data with date filtering
 */
const getProductionViewDataWithFilter = async (startDate, endDate, departmentId) => {
    try {
        const baseData = await (0, exports.getProductionViewData)();
        ...
        if (startDate || endDate) {
            ...
            filteredData.all_sub_batches = baseData.all_sub_batches.filter(sb => {
                ...
                if (startDate && sbStartDate < startDate)
                    return false;
                if (endDate && sbStartDate > endDate)
                    return false;
                return true;
            });
        ...
        if (departmentId) {
            ...
            filteredData.department_columns = baseData.department_columns.filter(dc => dc.department_id === departmentId);
        }
...
exports.getProductionViewDataWithFilter = getProductionViewDataWithFilter;
