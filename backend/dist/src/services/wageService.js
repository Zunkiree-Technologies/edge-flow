...
exports.getSubBatchWageSummary = exports.getDepartmentWageSummary = exports.getBillableWorkLogs = exports.calculateAllWorkersWages = exports.calculateWorkerWages = void 0;
// src/services/wageService.ts
const db_1 = __importDefault(require("../config/db"));
/**
 * Calculate wages for a specific worker within a date range
 */
const calculateWorkerWages = async (workerId, startDate, endDate) => {
    // Build where clause
    const whereClause = {
        worker_id: workerId,
    };
    if (startDate || endDate) {
        ...
        if (startDate)
            whereClause.work_date.gte = startDate;
        if (endDate)
            whereClause.work_date.lte = endDate;
    }
    // Get all logs for this worker
    const logs = await db_1.default.worker_logs.findMany({
        where: whereClause,
        include: {
            sub_batch: {
                select: {
...
exports.calculateWorkerWages = calculateWorkerWages;
/**
 * Calculate wages for all workers in a date range
 */
const calculateAllWorkersWages = async (startDate, endDate, departmentId) => {
    // Build where clause
    const whereClause = {};
    if (startDate || endDate) {
        whereClause.work_date = {};
...
exports.calculateAllWorkersWages = calculateAllWorkersWages;
/**
 * Get only billable work logs
 */
const getBillableWorkLogs = async (workerId, startDate, endDate) => {
    const whereClause = {
        is_billable: true,
    };
    if (workerId)
...
exports.getBillableWorkLogs = getBillableWorkLogs;
/**
 * Get department-wise wage summary
 */
const getDepartmentWageSummary = async (departmentId, startDate, endDate) => {
    // Get all workers in this department
    const workers = await db_1.default.workers.findMany({
    ...
    const workerWages = await (0, exports.calculateAllWorkersWages)(startDate, endDate, departmentId);
    const totalBillableWages = workerWages.reduce((sum, w) => sum + w.total_billable_wages, 0);
    const totalNonBillableWages = workerWages.reduce((sum, w) => sum + w.total_non_billable_wages, 0);
    return {
        department_id: departmentId,
...
exports.getDepartmentWageSummary = getDepartmentWageSummary;
/**
 * Get wage summary by sub-batch
 */
const getSubBatchWageSummary = async (subBatchId) => {
    const logs = await db_1.default.worker_logs.findMany({
...
exports.getSubBatchWageSummary = getSubBatchWageSummary;
