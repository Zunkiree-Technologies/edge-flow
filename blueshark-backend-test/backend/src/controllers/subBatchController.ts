...
// Check sub-batch dependencies for bulk delete
export const checkDependencies = async (req: Request, res: Response) => {
  try {
    const { subBatchIds } = req.body;

    // Validation
    if (!subBatchIds || !Array.isArray(subBatchIds) || subBatchIds.length === 0) {
      return res.status(400).json({
        message: "Invalid request. subBatchIds must be a non-empty array.",
      });
    }

    // Ensure all IDs are numbers
    const validIds = subBatchIds.filter((id) => typeof id === "number" && id > 0);
    if (validIds.length !== subBatchIds.length) {
      return res.status(400).json({
        message: "All sub-batch IDs must be valid positive numbers.",
      });
    }

    const result = await subBatchService.checkSubBatchDependencies(validIds);
    res.json(result);
  } catch (err: any) {
    res.status(500).json({ message: "Error checking dependencies", error: err.message || err });
  }
};