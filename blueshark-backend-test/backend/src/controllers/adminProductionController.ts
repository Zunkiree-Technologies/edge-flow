...
export const rejectSubBatch = async (req: Request, res: Response) => {
  try {
    const { sub_batch_id, from_department_id, quantity, reason, worker_log_id } = req.body;

    // Validation - return_to_department_id removed (rejection = waste/scrap, no new card)
    if (!sub_batch_id || !from_department_id || !quantity || !reason || !worker_log_id) {
      return res.status(400).json({
        success: false,
        message: "All fields are required: sub_batch_id, from_department_id, quantity, reason, worker_log_id",
      });
    }
    ...
    const result = await adminProductionService.createRejection({
      ...
      reason,
      worker_log_id: Number(worker_log_id),
    });

    res.status(201).json({
      success: true,
      message: "Rejection recorded as waste/scrap successfully",
      data: result,
    });
  } catch (err: any) {
    res.status(500).json({
...
export const addAlteration = async (req: Request, res: Response) => {
  try {
    const { sub_batch_id, from_department_id, return_to_department_id, quantity, note, worker_log_id } = req.body;

    // Validation
    if (!sub_batch_id || !from_department_id || !return_to_department_id || !quantity || !note || !worker_log_id) {
      return res.status(400).json({
        success: false,
        message: "All fields are required: sub_batch_id, from_department_id, return_to_department_id, quantity, note, worker_log_id",
      });
    }
