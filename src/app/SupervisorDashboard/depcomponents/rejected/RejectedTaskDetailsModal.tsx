...
const RejectedTaskDetailsModal: React.FC<RejectedTaskDetailsModalProps> = ({
  ...
  const handleAddWorker = async () => {
    ...
    if (quantity > remainingWork) {
      showToast(
        "error",
        `Cannot assign ${quantity} units! Only ${remainingWork} units remaining from rejected quantity of ${receivedQuantity}. Already assigned: ${workedQuantity} units`
      );
      return;
    }
  ...
  const handleSaveEdit = async (workerId: number) => {
    ...
    if (quantity > remainingWork) {
      showToast(
        "error",
        `Cannot assign ${quantity} units! Only ${remainingWork} units available (excluding current assignment). Total rejected: ${receivedQuantity}, Other workers: ${otherWorkersQuantity} units`
      );
      return;
    }
  ...
  const handleSave = async () => {
    ...
    if (taskData.status === "COMPLETED" && status === "COMPLETED" && sendToDepartment) {
      try {
        ...
        const totalWorkedQuantity = workerRecords.reduce(
              ...
              <div className="mb-8">
                ...
                <div className="flex items-start gap-12">
                  ...
                  <div className="flex flex-col">
                    ...
                    <p className="text-[16px] text-center font-semibold text-gray-900">
                      {(
                        (taskData.quantity_remaining ?? taskData.rejected_quantity) -
                        workerRecords.reduce((sum, record) => sum + (record.quantity || 0), 0)
                      ).toLocaleString()}
                    </p>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-4 mb-4">
                  ...
                  {newWorkerId && (
                    <div className="flex items-center gap-2">
                      ...
                      />
                      <label
                        htmlFor="billable-checkbox-rejected"
                  ...
                  <div className="space-y-3 relative">
                    {subBatchHistory.department_flow
                      ...
                      .map((deptName: string, index: number) => {
                        ...
                        const isRejectedCurrentDepartment =
                        ...
                        const isMainSubBatchHere =
                          deptDetail &&
                          (!nextDept || !nextDept.worker_logs || nextDept.worker_logs.length === 0);
