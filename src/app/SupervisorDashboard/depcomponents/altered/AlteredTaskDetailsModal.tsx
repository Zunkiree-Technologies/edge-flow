...
const AlteredTaskDetailsModal: React.FC<AlteredTaskDetailsModalProps> = ({
    ...
    try {
      ...
      if (result.success && Array.isArray(result.data)) {
        ...
        const filteredData = result.data.filter((r: any) => {
          ...
          const isThisCard =
            r.department_sub_batch_id &&
            currentDeptSubBatchId &&
  ...
  const handleAddWorker = async () => {
    ...
    if (remainingWork <= 0) {
      showToast(
        "error",
        "Cannot add worker! There is no remaining work. All received quantity has been assigned to workers."
      );
      return;
    }
    ...
    if (quantity > remainingWork) {
      showToast(
        "error",
        `Cannot assign ${quantity} units! Only ${remainingWork} units remaining from altered quantity of ${receivedQuantity}. Already assigned: ${workedQuantity} units`
      );
      return;
    }
    ...
    try {
      ...
      showToast(
        "error",
        `Error saving record! ${e instanceof Error ? e.message : "Unknown error"}`
      );
    } finally {
      setSaving(false);
    }
  ...
  const handleSaveEdit = async (workerId: number) => {
    ...
    if (quantity > remainingWork) {
      showToast(
        "error",
        `Cannot assign ${quantity} units! Only ${remainingWork} units available (excluding current assignment). Total altered: ${receivedQuantity}, Other workers: ${otherWorkersQuantity} units`
      );
      return;
    }
    ...
    if (taskData.status === "COMPLETED" && status === "COMPLETED" && sendToDepartment) {
      try {
        ...
        if (quantity > availableQuantity) {
          showToast(
            "error",
            `Cannot send ${quantity} pieces! Only ${availableQuantity} pieces available. Enter between 1 and ${availableQuantity}.`
          );
          setSaving(false);
          return;
        }
        ...
        showToast(
          "error",
          `Failed to send to department: ${error instanceof Error ? error.message : "Unknown error"}`
        );
      } finally {
        setSaving(false);
      }
      ...
      if (status === "COMPLETED") {
        const receivedQuantity = taskData.quantity_remaining ?? taskData.altered_quantity;
        const workedQuantity = workerRecords.reduce(
        ...
        if (remainingWork > 0) {
          showToast(
            "error",
            `Cannot mark as Completed! You still have ${remainingWork} pieces remaining. Please assign all ${receivedQuantity} pieces to workers.`
          );
          return;
        }
      }
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-xs text-yellow-800 leading-relaxed">
                      This card contains{" "}
                      <span className="font-semibold">{taskData.altered_quantity} pieces</span> that
                      were sent back from{" "}
                      <span className="font-semibold">{taskData.altered_by}</span> for rework due
                      to:{" "}
                      <span className="font-semibold italic">
                        &quot;{taskData.alter_reason || "quality issues"}&quot;
                      </span>
                    </p>
                ...
                <div className="space-y-6">
                  ...
                  <div className="grid grid-cols-2 gap-6">
                    ...
                    <div>
                      ...
                      <div className="bg-gray-50 border border-gray-200 rounded-lg px-4 py-2.5 text-sm text-gray-600">
                        {(
                          taskData.original_quantity ||
                          taskData.sub_batch?.estimated_pieces ||
                          taskData.total_quantity
                        ).toLocaleString()}
                      </div>
                    </div>
                  </div>
                          ...
                          <select
                            ...
                            onChange={(e) => {
                              ...
                              if (e.target.value && !quantityBeingSent) {
                                setQuantityBeingSent(
                                  (
                                    taskData.quantity_received ?? taskData.altered_quantity
                                  ).toString()
                                );
                              }
                            }}
                            ...
                            {departments
                              .filter((dept) => {
                                ...
                                return currentDeptId
                                  ? dept.id.toString() !== String(currentDeptId)
                                  : true;
                              })
                              .map((dept) => (
                ...
                <div className="bg-white p-4">
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div className="bg-yellow-50 rounded-lg p-3 border border-yellow-200">
                      ...
                      <div className="text-sm text-yellow-900 font-bold">{taskData.altered_by}</div>
                    </div>
                        ...
                        .map((dept: any) => {
                          ...
                          return (
                            <div
                              ...
                              className="border border-gray-300 rounded-lg w-[450px]"
                            >
                              <button
                                onClick={() => {
                                  if (isExpanded) {
            ...
            <div className="px-4 py-6 bg-gray-50">
              ...
              <div className="mb-4">
                <p className="text-sm font-medium text-gray-900 break-words">
                  {taskData.batch_name || "Linen Silk"}
                </p>
                <p className="text-xs text-gray-500 break-words">
                  {taskData.sub_batch_name || "B001.1"}
                </p>
              </div>
              ...
              {subBatchHistory && subBatchHistory.department_flow ? (
                      ...
                      .map((deptName: string, index: number) => {
                        ...
                        return (
                          <div key={index} className="flex items-start gap-2 relative">
                            <div
                              className={`w-[8px] h-[8px] rounded-full border-2 z-10 mt-1 flex-shrink-0 ${dotClasses}`}
                            />
                ...
                <div className="text-sm text-gray-400">Loading route details...</div>
              )}
            </div>
          </div>
