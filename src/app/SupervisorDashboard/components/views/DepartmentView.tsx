...
const SupervisorKanban = () => {
  ...
  const sortItems = useCallback(
    (items: WorkItem[]) => {
      ...
      switch (sortBy) {
        case "name":
          return sortedItems.sort((a, b) => a.sub_batch.name.localeCompare(b.sub_batch.name));
        case "startDate":
          return sortedItems.sort(
            (a, b) =>
              new Date(a.sub_batch.start_date).getTime() -
              new Date(b.sub_batch.start_date).getTime()
          );
        case "dueDate":
          return sortedItems.sort(
            (a, b) =>
  ...
  const fetchKanbanData = useCallback(async () => {
    try {
      ...
      const targetDeptId =
        isSuperSupervisor && typeof selectedDepartmentId === "number"
  ...
  const alteredTaskData = useMemo(() => {
    ...
    return {
      ...
      total_quantity:
        selectedItem.quantity_remaining ?? selectedItem.sub_batch?.estimated_pieces ?? 0,
      ...
      sent_from_department:
        selectedItem.alteration_source?.from_department_name ||
      ...
      sent_from_department_id:
        selectedItem.alteration_source?.from_department_id || selectedItem.sent_from_department,
      alteration_date:
        selectedItem.alteration_source?.created_at ||
      ...
      altered_quantity:
        selectedItem.alteration_source?.quantity || selectedItem.quantity_received || 0,
      alter_reason:
        selectedItem.alteration_source?.reason ||
        selectedItem.alter_reason ||
        selectedItem.remarks ||
        "",
      attachments:
        selectedItem.sub_batch?.attachments?.map((att: Attachment) => ({
  ...
  const rejectedTaskData = useMemo(() => {
    ...
    return {
      ...
      sent_from_department:
        selectedItem.rejection_source?.from_department_name ||
        selectedItem.sent_from_department_name ||
        selectedItem.sent_from_department ||
        "Unknown",
      rejection_date:
        selectedItem.rejection_source?.created_at ||
        selectedItem.createdAt ||
        new Date().toISOString(),
      ...
      rejected_quantity:
        selectedItem.rejection_source?.quantity || selectedItem.quantity_remaining || 0,
  ...
  const totalTaskCount = useMemo(() => {
    return (
      (kanbanData.newArrival?.length || 0) +
      ...
      <div className="p-8 bg-gray-50 min-h-full">
        ...
        <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-12">
          <div className="text-center">
            ...
            <h3 className="text-xl font-semibold text-gray-900 mb-3">Select a Department</h3>
            <p className="text-gray-600 max-w-md mx-auto mb-6">
              Please select a specific department from the dropdown above to view and manage tasks
              in the Kanban board.
            </p>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
          {updatedStages.map((stage) => {
            ...
            return (
              <div
                key={stage.key}
                className={`${stage.color} rounded-xl overflow-hidden flex flex-col h-full`}
              >
                ...
                <div className="px-4 py-3 bg-white border border-gray-200 rounded-lg mx-3 mt-3">
                  <div className="flex items-center justify-between">
                    ...
                    <span
                      className={`text-xs font-medium px-2 py-0.5 rounded-full ${stage.badgeColor}`}
                    >
                      {stage.count}
                    </span>
                  </div>
                ...
                <div className="p-3 space-y-2 flex-1 overflow-y-auto min-h-[400px] max-h-[600px]">
                  {items.length === 0 ? (
                    ...
                    items.map((item) => {
                      ...
                      const isAlteredByRemarks =
                        item.remarks?.toLowerCase().includes("alter") ?? false;
                      const isAssigned = item.remarks === "Assigned";
                      const isNewArrival =
                        stage.key === "newArrival" &&
                        !isRejected &&
                        !isAlteredByRemarks &&
                        !isAssigned;
                      ...
                      const isReworkCard =
                        item.alteration_source !== null && item.alteration_source !== undefined;
                      ...
                      return (
                        ...
                        >
                          ...
                          <span
                            className={`absolute top-3 right-3 inline-block px-3 py-1 text-xs rounded-md font-medium ${
                          ...
                          {isAssigned && item.assigned_worker && (
                            <div className="flex items-center gap-2 text-sm text-blue-700 mb-2 font-medium">
                              <span className="text-xs">
                                👷 Worker: {item.assigned_worker.name}
                              </span>
                            </div>
                          )}
                          ...
                          <div className="mb-2 space-y-1">
                            ...
                            <div className="flex items-center gap-2 text-sm text-gray-600">
                              ...
                              <span className="text-xs font-medium">
                                Remaining:{" "}
                                {(
                                  item.quantity_remaining ?? item.sub_batch.estimated_pieces
                                ).toLocaleString()}{" "}
                                pcs
                              </span>
                            </div>
                            ...
                            {(() => {
                              const received =
                                item.quantity_received ?? item.sub_batch.estimated_pieces;
                              const remaining =
                                item.quantity_remaining ?? item.sub_batch.estimated_pieces;
                          ...
                          <div className="flex items-center gap-2 text-sm text-gray-600 mb-2">
                            ...
                            <span className="text-xs">
                              Start: {formatNepaliDate(item.sub_batch.start_date)}
                            </span>
                          </div>
                          ...
                          <div className="flex items-center gap-2 text-sm text-gray-600 mb-2">
                            ...
                            <span className="text-xs">
                              Due: {formatNepaliDate(item.sub_batch.due_date)}
                            </span>
                          </div>
