  ...
  const performSearch = async () => {
    ...
    try {
      ...
      try {
        ...
        const filteredRolls = rollsRes.data.filter(
          (roll: { id: number; name?: string; color?: string }) =>
            roll.name?.toLowerCase().includes(searchQuery.toLowerCase())
        );
        results.push(
          ...filteredRolls.map((roll: { id: number; name: string; color?: string }) => ({
            id: roll.id,
            type: "roll" as const,
            name: roll.name,
      ...
      try {
        ...
        const filteredBatches = batchesRes.data.filter(
          (batch: { id: number; name?: string; quantity?: number; unit?: string }) =>
      ...
      try {
        ...
        const filteredSubBatches = subBatchesRes.data.filter(
          (sb: { id: number; name?: string; estimated_pieces?: number }) =>
            sb.name?.toLowerCase().includes(searchQuery.toLowerCase())
        );
        results.push(
          ...filteredSubBatches.map(
            (sb: { id: number; name: string; estimated_pieces?: number }) => ({
              id: sb.id,
      ...
      try {
        ...
        const filteredVendors = vendorsRes.data.filter(
          (vendor: { id: number; name?: string; phone?: string; address?: string }) =>
      ...
      try {
        ...
        const filteredWorkers = workersRes.data.filter(
          (worker: { id: number; name?: string; position?: string; department?: string }) =>
      ...
      try {
        ...
        results.push(
          ...filteredDepts.map((dept: { id: number; name: string }) => ({
            id: dept.id,
    ...
    <header className="bg-[#f7f7f7] px-6 py-3 relative h-[60px] flex items-center gap-4 w-full">
      ...
      <div className="relative w-[500px]">
        <div className="flex items-center bg-white rounded-xl px-4 py-2 border border-gray-300 focus-within:border-blue-400 focus-within:ring-2 focus-within:ring-blue-100 transition-all">
          ...
          {isSearching && <div className="ml-2 text-xs text-gray-500">Searching...</div>}
        </div>
              ...
              {searchResults.map((result, index) => (
                ...
                >
                  <div className="flex items-center justify-between">
                    ...
                    <span
                      className={`text-xs px-2 py-1 rounded-full ${getTypeBadgeColor(result.type)}`}
                    >
