...
const ProductionTaskDetailsModal: React.FC<ProductionTaskDetailsModalProps> = ({
  ...
  const fetchAvailableWorkers = useCallback(async () => {
    ...
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/workers/department/${taskData.department_id}`
      );
      if (response.ok) {
  ...
  const handleStatusChange = async (newStatus: string) => {
    ...
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/admin/production/update-status`,
        {
          method: "PUT",
                    ...
                    <div>
                      ...
                      <div className="flex flex-wrap gap-2">
                        {taskDetails.attachments.map((att: any, idx: number) => (
                          <span
                            key={idx}
                            className="px-3 py-1.5 bg-gray-100 text-sm text-gray-700 rounded border border-gray-200"
                          >
                    ...
                    <div className="space-y-3">
                      {taskDetails.route_steps.map((step: any, idx: number) => (
                        <div key={idx} className="flex items-center gap-3">
                          ...
                          />
                          <span className="text-sm text-gray-900">{step.department_name}</span>
                        </div>
                      ))}
                    </div>
                  ) : (
